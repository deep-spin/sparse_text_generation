...
def pad_dataset(dataset, padding=0):
    ...
    for name in PADDED_INPUTS:
        dataset[name] = [x + [padding if name != "lm_labels" else padding] * (max_l - len(x)) for x in dataset[name]]
    return dataset
...
def build_input_from_segments(persona, history, reply, tokenizer, lm_labels=False, with_eos=True):
    ...
    instance["lm_labels"] = [50259] * len(instance["input_ids"])
    if lm_labels:
        instance["lm_labels"] = ([50259] * sum(len(s) for s in sequence[:-1])) + [50259] + sequence[-1][1:]
    return instance
...
def train():
    ...
    metrics = {"nll": Loss(loss_func(ignore_index=-1), output_transform=lambda x: (x[0][0], x[1][0])),
    ...
    def checkpoint(trainer):
        torch.save(model.state_dict(), log_dir + '/model_' +str(trainer.state.epoch) + '.bin')
