...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    """
    def __init__(self, config, loss=CrossEntropyLoss, gen_func=None):
    ...
    def batch_input_sequence_by_prefix_length(self, input_sequence, prefix_length):
    ...
    def softmax_temperature(self, X, temperature = 1.0, axis = None):
        X=X.squeeze(0)
    ...
    def forward(self, input_ids, unlikelihood=False, unlikelihood_seq=False, past=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None, labels=None, tokenizer=None):
...
class GPT2DoubleHeadsModel(GPT2PreTrainedModel):
    ...
    Outputs: `Tuple` comprising various elements depending on the configuration (config) and inputs:
        ...
        **past**:
            list of ``torch.FloatTensor`` (one for each layer) of shape ``(batch_size, num_heads, sequence_length, sequence_length)``:
            that contains pre-computed hidden-states (key and values in the attention blocks).
            Can be used (see `past` input) to speed up sequential decoding.past
        **hidden_states**: (`optional`, returned when ``config.output_hidden_states=True``)
            list of ``torch.FloatTensor`` (one for the output of each layer + the output of the embeddings)
            of shape ``(batch_size, sequence_length, hidden_size)``:
            Hidden-states of the model at the output of each layer plus the initial embedding outputs.
    ...
    """
    def __init__(self, config, loss=CrossEntropyLoss):
    ...
    def forward(self, input_ids, past=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
        ...
        if mc_labels is not None:
            loss = self.loss(mc_logits.view(-1, mc_logits.size(-1)),mc_labels.view(-1))
            outputs = (loss,) + outputs
